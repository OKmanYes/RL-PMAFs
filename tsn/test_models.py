...
parser.add_argument('dataset', type=str, choices=['ucf101', 'hmdb51', 'kinetics','pma'])
...
data_loader = torch.utils.data.DataLoader(
        TSNDataSet("", args.test_list, num_segments=args.test_segments,
                   ...
                   image_tmpl="img_{:05d}.jpg" if args.modality in ['RGB', 'RGBDiff'] else args.flow_prefix+"{:05d}.png",
                   test_mode=True,
...
def eval_video(video_data):
    ...
    input_var = torch.autograd.Variable(data.view(num_crop, args.test_segments, length, data.size(2), data.size(3)), volatile=True)
