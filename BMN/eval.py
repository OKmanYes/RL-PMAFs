...
def run_evaluation(ground_truth_filename, proposal_filename, 
                   ...
                   max_avg_nr_proposals=40,#                                                   2021.7.11
...
def evaluation_proposal(opt):
    ...
    print( "AR@1 is \t",np.mean(uniform_recall_valid[:,0]))  #1的时候为0，原因可能是原来100*某个比例 算这个proposal数目，现在1的时候是40*0.01取值小于1了
    print( "AR@5 is \t",np.mean(uniform_recall_valid[:,4]))
    print( "AR@10 is \t",np.mean(uniform_recall_valid[:,9]))
    print( "AR@40 is \t",np.mean(uniform_recall_valid[:,-1]))