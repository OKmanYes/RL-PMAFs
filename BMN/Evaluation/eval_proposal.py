...
class ANETproposal(object):
    ...
    def _import_proposal(self, proposal_filename):
        ...
        """
        print(proposal_filename)
        with open(proposal_filename, 'r') as fobj:
            data = json.load(fobj)
            #print(data)  #result 文件
        # Checking format...
        ...
        for videoid, v in data['results'].items():
            #print(data['results'])                                             # result.json文件
            if videoid in self.blocked_videos:
                continue
            for result in v:
                #print(v)                                #那个文件去掉 02e名字剩下的
                video_lst.append(videoid)
...
def average_recall_vs_avg_nr_proposals(ground_truth, proposals,
    ...
    print('proposals.shape[0]')#2200
    print(proposals.shape[0])
    print('video_lst.shape[0]')#55
    print(video_lst.shape[0])

    if not max_avg_nr_proposals:
        max_avg_nr_proposals = float(proposals.shape[0])/video_lst.shape[0]

    print('max_avg_nr_proposals')    #2200  55  40  是40
    print(max_avg_nr_proposals)
    ...
    for videoid in video_lst:
        ...
        #print('this_video_proposals.shape[0]')            #40
        #print(this_video_proposals.shape[0])
        #print('this_video_proposals.shape[0] * ratio')
        #print(this_video_proposals.shape[0] * ratio)
        nr_proposals = np.minimum(int(this_video_proposals.shape[0] * ratio), this_video_proposals.shape[0])
        total_nr_proposals += nr_proposals         #每次+40  总计2200
        #print('total_nr_proposals')   #11111111111111111111111111111111111111111
        #print(total_nr_proposals)
    ...
    pcn_lst = np.arange(1, 41) / 40.0 *(max_avg_nr_proposals*float(video_lst.shape[0])/total_nr_proposals)# 1111111111111111111111111111
    #pcn_lst = np.arange(1, 41) / 100.0 *(max_avg_nr_proposals*float(video_lst.shape[0])/total_nr_proposals)# 1111111111111111111111111111
    ...
    for ridx, tiou in enumerate(tiou_thresholds):
        ...
        for i, score in enumerate(score_lst):
            ...
            #print('pcn_lst')#                111111111111111111111111111111  0.01  0.02...0.40
            #print(pcn_lst)
            #print('score.shape[1]')   #        111111111111111111111
            #print(score.shape[1])       #40
