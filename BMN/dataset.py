...
class VideoDataSet(data.Dataset):
    ...
    def _getDatasetDict(self):
        anno_df = pd.read_csv(self.video_info_path)     #      2021.8.10   
        ...
        print("%s subset video numbers: %d" % (self.subset, len(self.video_list)))#第一行打印的 train视频有多少
...
#   data/activitynet_feature_cuhk/csv_mean_100/video_name.csv
        video_data = video_df.values[:, :]#取所有行所有列
    ...
    def _get_train_label(self, index, anchor_xmin, anchor_xmax):
        ...
        ####################################################################################################
        # generate R_s and R_e  start region   end region
        gt_bbox = np.array(gt_bbox)
        gt_xmins = gt_bbox[:, 0]
        gt_xmaxs = gt_bbox[:, 1]
        gt_lens = gt_xmaxs - gt_xmins
        gt_len_small = 3 * self.temporal_gap  # np.maximum(self.temporal_gap, self.boundary_ratio * gt_lens)    #8.12
        #gt_len_small = self.temporal_gap  # np.maximum(self.temporal_gap, self.boundary_ratio * gt_lens)
        ...
        for jdx in range(len(anchor_xmin)):
            ...
            #print(anchor_xmin[jdx])
            #print(anchor_xmax[jdx])
            #print(match_score_start)
        match_score_end = []
        for jdx in range(len(anchor_xmin)):
            ...
            #print('anchor_xmax[jdx]')
            #print(anchor_xmax[jdx])
            #print(match_score_end)
        #print('gt_end_bboxs[:, 0]')
        #print(gt_end_bboxs[:, 0])
        #print('gt_end_bboxs[:, 1]')
        #print(gt_end_bboxs[:, 1])
